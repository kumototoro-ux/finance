...
/* -------------------- الدفعات (على أساس الاستحقاقات، لا الفواتير) -------------------- */
export const attachmentSchema = z.object({
  url: z.string().url('رابط المرفق غير صحيح'),
  name: z.string().max(150).optional().or(z.literal('')),
});

/** توزيع يدوي اختياري لجزء من مبلغ الدفعة على استحقاق معيّن — لو أُرسِلت
 * قائمة توزيعات، يجب أن يساوي مجموعها مبلغ الدفعة بالضبط. بلا توزيع
 * يدوي، يوزَّع المبلغ تلقائياً على أقدم الاستحقاقات المفتوحة أولاً. */
export const paymentAllocationSchema = z.object({
  dueId: z.union([z.string(), z.number()]),
  amount: z.number().positive('مبلغ كل بند بالتوزيع يجب أن يكون أكبر من صفر'),
});
...
/* -------------------- الرواتب -------------------- */
export const setSalaryProfileSchema = z.object({
  employeeId: z.string().min(1, 'رقم الموظف مطلوب'),
  baseSalary: z.number().min(0, 'الراتب الأساسي يجب أن يكون صفراً أو أكبر'),
  defaultAllowances: z.number().min(0).default(0),
  defaultDeductions: z.number().min(0).default(0),
});

export const createPayrollRunSchema = z.object({
  branch: z.string().trim().optional().or(z.literal('')), // فارغ = دورة تشمل كل الفروع
  payMonth: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'صيغة الشهر غير صحيحة (أول يوم بالشهر)'),
});
